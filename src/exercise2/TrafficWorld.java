...
public class TrafficWorld extends World {
	...
	{    
		...
		getBackground().fill();
		createRoads();
		intersections = new IntersectionSet(this, ewRoads, nsRoads);
		createCars();
	}
	
	private EWRoad[] ewRoads = new EWRoad[N_EW_ROADS];
	private NSRoad[] nsRoads = new NSRoad[N_NS_ROADS];
	private IntersectionSet intersections;
	...
	public Intersection findApproachingIntersection(Car car) {
		return intersections.findApproachingIntersection(car);
	}

	public Intersection findContainingIntersection(Car car) {
		return intersections.findContainingIntersection(car);
	}
